...
export const usePhotoService = (
  ...
  const uploadPhoto = async () => {
    ...
    const requestPermissions = async () => {
      try {
        ...
        const isCameraApproved =
          permissionRequestResult['android.permission.CAMERA'] === PermissionsAndroid.RESULTS.GRANTED

        const isStorageApproved =
          permissionRequestResult['android.permission.WRITE_EXTERNAL_STORAGE'] === PermissionsAndroid.RESULTS.GRANTED
    ...
    const isWritingAllowed = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE)
    const isCameraAllowed = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE)
  ...
  useEffect(
    function whatToDoNextAfterPhotoUpload() {
