...
export const useRefreshMessageStack = (chatMiddleware: TUseChatMiddleware) => {
  ...
  useEffect(function scrollDownWhenKeyboardIsOpen() {
    ...
    const keyboardDidShowListener = Keyboard.addListener(keyboardShowListener, () => {
  ...
  useEffect(
    function needToProvokeABotMessage() {
      const isLastMessageTypedMe = messages.length && messages[messages.length - 1].sender === 'me'
  ...
  useEffect(
    function addNewBotMessageWhileStackIsNotEmpty() {
