...
const ChatDatepicker: FC<TChatProps> = React.memo(({ chatMiddleware, libraryInputData }) => {
  const { title, endFunc } = chatMiddleware!.currentChatBotQuestion!.myAnswer!.DATEPICKER!
  ...
  return (
    <View style={ChatDatepickerStyles.main}>
      <View style={ChatDatepickerStyles.datePicker}>
        <DatePicker onSaveDate={onSaveDate} mode={'bornDate'} viewStyles={libraryInputData.viewStyles} />
      </View>
      <ButtonComponent
        title={title}
