...
const ChatChoice: FC<TChatProps> = React.memo(({ chatMiddleware, libraryInputData }) => {
  const { checkboxTitles, endFunc } = chatMiddleware!.currentChatBotQuestion!.myAnswer!.CHOICE!
  const { answerFieldColor, buttonColor } = libraryInputData.viewStyles
  const [selected, refreshSelected] = React.useState<TCheckboxData>(checkboxTitles[0])
  const onValueChange = (item: TCheckboxData) => {
    refreshSelected(item)
  }

  const onPress = React.useCallback(() => {
    chatMiddleware.sendAnswer(selected.checkboxTitle.toUpperCase(), EBubbleType.TEXT)
      ...
      <View style={ChatChoiceStyles.scrollPickerWrapper}>
        <ScrollPicker
          ...
          renderItem={(data: TCheckboxData, isSelected: boolean) => (
            <Text style={[ChatChoiceStyles.scrollPickerText, { color: isSelected ? '#4F4E4E' : '#C3C3C3' }]}>
